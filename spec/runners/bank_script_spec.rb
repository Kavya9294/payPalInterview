...
describe BankScript do
  ...
  describe "run" do
    ...
    it "creates User with 0 balance" do
			allow_any_instance_of(Object).to receive(:open).and_return(content_create)
      expect{ described_class.run(args) }
      .to output("Tom: $0\n").to_stdout
    end

		it "creates User with charged balance" do
			allow_any_instance_of(Object).to receive(:open).and_return(content_charge)
      expect{ described_class.run(args) }
      .to output("Tom: $700\n").to_stdout
    end

		it "creates User with credited balance" do
			allow_any_instance_of(Object).to receive(:open).and_return(content_credit)
      expect{ described_class.run(args) }
      .to output("Tom: $-100\n").to_stdout
    end

		it "leads to error due to inavlid card" do
			allow_any_instance_of(Object).to receive(:open).and_return(content_error)
      expect{ described_class.run(args) }
      .to output("Tom: Error\n").to_stdout
    end
      ...
      .to output("Tom: $500\n").to_stdout
		...
		it "leads to error due to inavlid card and balance is not updated" do
			allow_any_instance_of(Object).to receive(:open).and_return(content_error_update)
      expect{ described_class.run(args) }
      .to output("Tom: Error\n").to_stdout
    end
